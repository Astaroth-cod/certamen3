...
def conectar():
    try:
        client = MongoClient("mongodb://localhost:27017", serverSelectionTimeotMs=3000)
        client.server_info()
        db =client["certamen 3"]
        return db
    except Exception as e:
        print(f"\n[Error] no se pude conectar a Mongo :c:{e}")
        exit(1)
# ACTIVIDAD PARTE 1#
...
def act1_lista(db):
    ...
    eventos = db.eventos.find({},
                              {"codigo":1,"nombre":1,"fecha":1,"lugar":1,"categoria":1,"_id":0}).sort("fecha",1)
    total = 0
    for ev in eventos:
...
def act1_eventos_por_categoria(db):
    ...
    eventos = db.eventos.find(
        {"categoria": cat},
        {"codigo": 1, "nombre": 1, "fecha": 1, "lugar": 1, "_id": 0}
    )
    total = 0
    for ev in eventos:
        fecha_str = ev["fecha"].strftime("%d/%m/%Y %H:%M") if isinstance(ev["fecha"], datetime) else str(ev["fecha"])
...
def act2_buscar_evento_por_nombre(db):
    """Busca eventos cuyo nombre contenga un término (regex)."""
    ...
    eventos = db.eventos.find(
        {"nombre": {"$regex": patron}},
        {"codigo": 1, "nombre": 1, "fecha": 1, "lugar": 1, "categoria": 1, "_id": 0}
    )
...
def act3_verificar_acceso(db):
    ...
    invitado = db.invitados.find_one(
        {"rut": rut, "estado": "activo"},
        {"nombre": 1, "empresa": 1, "_id": 0}
    )
...
def menu_principal(db):
    while True:
        cabeza()
        print("\n  MENÚ PRINCIPAL\n")
        print("  [1] Actividad 1 · Filtros y Condiciones")
        print("  [2] Actividad 2 · Expresiones Regulares")
        print("  [3] Actividad 3 · Búsquedas en Subdocumentos")
        print("  [4] Actividad 4 · $lookup y Agregaciones")
        print("  [0] Salir")
        op = input("\n  Selecciona: ").strip()
        if op == "1": menu1(db)
        elif op == "2": menu2(db)
